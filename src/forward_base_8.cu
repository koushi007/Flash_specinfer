...
void forward_base_8_kernel(
    ...
    for (int j = 0; j < Tc; ++j) { // j is the column tile index
        ...
        // We wanna multiply (32 x D) = Qi with (32 x D) = Kj i.e., Qi * Kj^T
        // using tiles of size (8 x 4)
        float QiReg[8][T]; float KjReg[4][T];
        ...
        for(int sK = 0; sK < D; sK += T){
            int sI = 8 * tileX, sJ = sK;
            for(int I = 0; I < 8; I++){
                for(int J = 0; J < T; J++){
                    QiReg[I][J] = QiT[(sJ + J) * 32 + (sI + I)];
                }
            }
            ...
            for(int I = 0; I < 8; I++){
                for(int K = 0; K < 4; K++){
                    for(int J = 0; J < T; J++){
                        sumReg[I][K] += QiReg[I][J] * KjReg[K][J];
                    }
                }
            }
        }
        for(int I = 0; I < 8; I++){
            for(int J = 0; J < 4; J++){
                int sI = 8 * tileX + I;
                int sJ = 4 * tileY + J;
                ST[(sJ * Br) + sI] = sumReg[I][J] * softmax_scale;
            }
        }
        
        float row_m = -INFINITY;
        for (int y = 0; y < Bc; y++) {
            bool mask = CausalMask[bx*gridDim.y*N*N + by*N*N + (Bc*N * tileID) + (tx * N) + (j * Bc) + y];
...
std::vector<torch::Tensor> forward_base_8(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
    ...
    forward_base_8_kernel<<<grid_dim, block_dim, sram_size>>>(

...
void forward_3_kernel(
    ...
    for (int j = 0; j < Tc; ++j) { // j is the column tile index
        ...
        for (int y = 0; y < Bc; y++) {
            bool mask = (!IsTree) || ((startI[tx] >= startJ[y]) && (endI[tx] <= endJ[y]));
            // causal mask
...
std::vector<torch::Tensor> forward_3(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
    ...
    forward_3_kernel<<<grid_dim, block_dim, sram_size>>>(

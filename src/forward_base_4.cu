...
std::vector<torch::Tensor> forward_base_4(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
    ...
    const int sram_size =
        ...
        + (Bc * Br * sizeof(float)); // SRAM for causal mask
    ...
    forward_base_4_kernel<<<grid_dim, block_dim, sram_size>>>(
